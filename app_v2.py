...
class ModernGUI(tk.Tk):
    ...
    def _draw_search_button(self, hover=False):
        ...
        c.create_text(70, 24, text=label, fill='white', font=('SF Pro Display', 13, 'bold'))

    def _draw_restart_button(self, hover=False):
        """Draw the restart button"""
        c = self.restart_canvas
        c.delete('all')
        if not self.is_searching:
            # Disabled state
            color1, color2 = '#404040', '#303030'
            text_color = '#666666'
        else:
            color1, color2 = ('#FFA500', '#FF8C00') if hover else ('#FF9500', '#E6851A')
            text_color = 'white'
        ...
        for i in range(steps):
            ...
            c.create_rectangle(2, 2 + (44/steps)*i, 218, 2 + (44/steps)*(i+1),
                            fill=f'#{r:02x}{g:02x}{b:02x}', outline="")
        c.create_text(70, 24, text="Restart", fill=text_color, font=('SF Pro Display', 11, 'bold'))

    def _draw_abort_button(self, hover=False):
        ...
        if not self.is_searching:
            ...
            color1, color2 = ('#FF4444', '#CC3333') if hover else ('#FF3333', '#CC2222')
            text_color = 'white'
        ...
        for i in range(steps):
            ...
            c.create_rectangle(2, 2 + (44/steps)*i, 218, 2 + (44/steps)*(i+1),
                            fill=f'#{r:02x}{g:02x}{b:02x}', outline="")
        c.create_text(70, 24, text="Abort", fill=text_color, font=('SF Pro Display', 11, 'bold'))

    def abort_scraping(self):
        """Abort the current scraping operation"""
        # Add your abort logic here - might involve setting a flag 
        # that your background thread checks
        self.is_searching = False
        self.status_var.set("Scraping aborted")
        self.hide_progress()
        self._draw_search_button()
        self._draw_restart_button()
        self._draw_abort_button()

    def restart_scraping(self):
        """Restart the scraping with current settings"""
        if self.is_searching:
            self.abort_scraping()
        # Small delay to ensure cleanup, then restart
        self.after(100, self.on_run)
    ...
    def _build_modern_ui(self):
        ...
        # === Search buttons (three buttons side by side) — smaller paddings ===
        button_frame = tk.Frame(content_frame, bg=self.colors['bg'])
        button_frame.pack(fill='x', pady=(2, 0))

        buttons_container = tk.Frame(button_frame, bg=self.colors['bg'])
        buttons_container.pack(anchor='center', pady=(0, 3))

        # Main Scrape button
        self.search_canvas = tk.Canvas(buttons_container, width=140, height=48,
                                    bg=self.colors['bg'], highlightthickness=0, cursor='hand2')
        self.search_canvas.pack(side='left', padx=(0, 8))
        
        # Restart button  
        self.restart_canvas = tk.Canvas(buttons_container, width=140, height=48,
                                    bg=self.colors['bg'], highlightthickness=0, cursor='hand2')
        self.restart_canvas.pack(side='left', padx=4)
        
        # Abort button
        self.abort_canvas = tk.Canvas(buttons_container, width=140, height=48,
                                    bg=self.colors['bg'], highlightthickness=0, cursor='hand2')
        self.abort_canvas.pack(side='left', padx=(8, 0))
    ...
    def _setup_button_interactions(self):
        ...
        ### SEARCH BUTTON
        def _on_search_hover_in(_):
            if not self.is_searching:
                self._draw_search_button(True)

        def _on_search_hover_out(_):
            if not self.is_searching:
                self._draw_search_button(False)

        def _on_search_click(_=None):
            if self.is_searching:
                return
        ...
        self._draw_search_button()
        self.search_canvas.bind('<Enter>', _on_search_hover_in)
        self.search_canvas.bind('<Leave>', _on_search_hover_out)
        self.search_canvas.bind('<Button-1>', _on_search_click)
        self.bind('<Return>', _on_search_click)
        
        ### RESTART BUTTON
        def _on_restart_hover_in(_):
            if self.is_searching:
                self._draw_restart_button(True)

        def _on_restart_hover_out(_):
            if self.is_searching:
                self._draw_restart_button(False)

        def _on_restart_click(_=None):
            if not self.is_searching:
                return
            self.restart_scraping()
            
        self._draw_restart_button()
        self.restart_canvas.bind('<Enter>', _on_restart_hover_in)
        self.restart_canvas.bind('<Leave>', _on_restart_hover_out)
        self.restart_canvas.bind('<Button-1>', _on_restart_click)
        
        ### ABORT BUTTON
        def _on_abort_hover_in(_):
            if self.is_searching:
                self._draw_abort_button(True)

        def _on_abort_hover_out(_):
            if self.is_searching:
                self._draw_abort_button(False)

        def _on_abort_click(_=None):
            if not self.is_searching:
                return
            self.abort_scraping()
            
        self._draw_abort_button()
        self.abort_canvas.bind('<Enter>', _on_abort_hover_in)
        self.abort_canvas.bind('<Leave>', _on_abort_hover_out)
        self.abort_canvas.bind('<Button-1>', _on_abort_click)
    ...
    #         offset = self.animation_step * 0.1
            
    #         for x in range(0, width, 2):
    #             # Create wave effect
    #             wave = math.sin((x / width) * 4 * math.pi + offset) * 0.3 + 0.5
                
    #             # Interpolate colors
    #             r1, g1, b1 = int('66', 16), int('7E', 16), int('EA', 16)  # gradient_start
    #             r2, g2, b2 = int('76', 16), int('4B', 16), int('A2', 16)  # gradient_end
    ...
    #             color = f'#{r:02x}{g:02x}{b:02x}'
    #             self.gradient_canvas.create_rectangle(x, 0, x+2, height, 
    #                                                 fill=color, outline=color)
